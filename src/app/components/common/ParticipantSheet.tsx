...
export function ParticipantSheet({ open, onOpenChange, roomId }: ParticipantSheetProps) {
            ...
            {participants.map((participant) => (
              <div key={participant.id} className="flex items-center">
                ...
                <div className="flex-1 ml-4">
                  ...
                  <p className="text-xs text-gray-medium">{participant.email}</p>
                </div>
                <Tag text="참여" />
              </div>
            ))}
            {participants.length === 0 && (
              <p className="text-center text-gray-medium pb-4">참여자가 없어요.</p>
            )}
          </div>
        )}
      </ParticipantSheetContent>
